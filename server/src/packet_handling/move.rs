...
pub async fn handle(payload: String) -> axum::http::StatusCode {
    ...
    loop {
        ...
        if _game_state.players[index].nick == splitted_payload[2] {
            break;
        }

        index+=1;
    }
    if _game_state.players[index].token != splitted_payload[3] {
    ...
    if splitted_payload[1] == "1" {
        direction += 8;
    } else if splitted_payload[1] != "0" {
        return axum::http::StatusCode::BAD_REQUEST;
    }
